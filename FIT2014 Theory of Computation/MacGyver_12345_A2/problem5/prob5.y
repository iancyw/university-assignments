...
expr     :    double    {  $$ = newQuaternion($1,0,0,0);  }
	 |    double 'i'  {  $$ = newQuaternion(0,$1,0,0);  }
	 |    double 'j'  {  $$ = newQuaternion(0,0,$1,0);  }
	 |    double 'k'  {  $$ = newQuaternion(0,0,0,$1);  }
	 |    'i'      {  $$ = newQuaternion(0,1,0,0);  }
	 |    'j'      {  $$ = newQuaternion(0,0,1,0);  }
	 |    'k'      {  $$ = newQuaternion(0,0,0,1);  }
         |    ROTATION '(' double ',' expr ')'    {  $$ = rotation($3,$5);  }
	 |    '(' expr ')'  {  $$ = $2;  }
	 |    '-'expr  {  $$ = diff(newQuaternion(0,0,0,0),$2);  }
