    ...
    def __setitem__(self, key, item):
        ...
        for _ in range(self.table_size):
            if self.table[position] is None:                    # found empty slot
                ...
                self.count += 1
                if self.count > (0.5*self.table_size):
                    self.rehash()
                return
...
class Freq:
    def __init__(self):
        """
        This method initialises a Freq class object.

        :return: none
        :pre: class HashTable is imported
        :post: two instance variables have been created. one is a HashTable and the second is a tuple of (None, None)
        :complexity: best and worst case: O(1)
        """
    ...
    def add_file(self, filename):
        """
        This method reads from a text file and inserts every word into the HashTable. It also keeps an update on the
        most frequent word and how many times that word has appeared.

        :param filename: the name of the file to be read from
        :return: none
        :pre: file must be exist in the same directory
        :post: self.max has been updated to (most frequent word, number of most frequent word), and the hashtable has
               been filled with key and item pairs
        :complexity: best and worst case: O(n), where n is length of the file
        """
    ...
    def rarity(self, word):
        """
        This method rates the rarity of a word in the HashTable.

        :param word: the word to be evaluated
        :return: returns 0, 1, 2, or 3 depending on the evaluation
        :pre: HashTable contains key, max_num is not (None, None)
        :post: none
        :complexity: best and worst case: O(1)
        """
        try:
            ...
            if occurrences >= (max_num / 100):
                return 0
            elif (max_num / 1000) > occurrences >= (max_num / 100):
                return 1
            elif occurrences < (max_num / 1000):
                return 2
        except KeyError:
            return 3
    ...
    def evaluate_frequency(self, other_filename):
        """
        This method takes the current frequency count and using that, evaluates the rareness of words in other_filename.

        :param other_filename: file to be read from and evaluated
        :return: returns a tuple of (common, uncommon, rare, errors), where all values in the tuple being percentages
                 of the word rarity in the other_filename
        :pre: none
        :post: returns a tuple
        ...
        x = Freq()
        x.add_file(other_filename)

        for i in range(x.word_frequency.table_size):
            if x.word_frequency.table[i] is not None:
                total_words += 1
                if self.rarity(x.word_frequency.table[i][0]) == 0:
                    common += 1
                elif self.rarity(x.word_frequency.table[i][0]) == 1:
                    uncommon += 1
                elif self.rarity(x.word_frequency.table[i][0]) == 2:
                    rare += 1
                elif self.rarity(x.word_frequency.table[i][0]) == 3:
                    errors += 1
        common = (common / total_words) * 100
        uncommon = (uncommon / total_words) * 100
        rare = (rare / total_words) * 100
        errors = (errors / total_words) * 100
